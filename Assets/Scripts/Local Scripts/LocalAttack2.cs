...
public class LocalAttack2 : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider col)
    {
        ...
        if (col.gameObject.tag == "Player" && enemyPlayer.GetComponent<PlayerMove1>().canBeHit == true && enemyPlayer.GetComponent<PlayerMove1>().iFrames == false)
        {
            ...
            enemyPlayer.GetComponent<PlayerMove1>().animations.SetTrigger("Take Damage");
        }

        // if player hits someone who is blocking and they are attacking htey will be pushed backwards 
        if (col.gameObject.tag == "Player" && enemyPlayer.GetComponent<PlayerMove1>().canBeHit == false && enemyPlayer.GetComponent<PlayerMove1>().iFrames == false)
        {    
